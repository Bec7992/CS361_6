...
def main():
	...
	copy_ecx_to_edi() # pust start of code address in edi
	eax_ff()
	copy_eax_to_ebx()
	...
	size = size + 3 + 38 + 30 + 1 + 1# size + xor length + longest to push size to stack + 2*cat_xor(b'\x00'[0]) + pop eax + push esp
	...
	print("alpha_code = " + str(len(alpha_code)))
	...
	if alpha_check(binary_byte[0]):
		...
		cat_not(binary_byte[0]) # 19 bytes long
